#!/usr/bin/env bash
# M6-C — restore proved by destroying and rebuilding. Chains from M6-B.
#
# After M6-B because a restore drill needs an archive, and an archive nobody has read back
# is not one this repository will restore from. It builds a THROWAWAY database and destroys
# that: tools/restore.py drops what it is pointed at, which is the only genuinely
# destructive operation here, and it is never pointed at the working database.
set -euo pipefail
...
bash "$REPO/tests/m6b/run_verification.sh"
...
echo
echo
echo "=== 26. M6-C verification gates ==="
"$PY_BIN" "$REPO/tests/m6c/verify_m6c.py"
