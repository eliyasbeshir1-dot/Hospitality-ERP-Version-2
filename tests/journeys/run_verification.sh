...
#
# Runs AFTER the slice suites, and reports its own outcome whether or not one of them
# failed. If a slice check and a journey fail together that is one signal; if only the
...
slice_status=0
bash "$REPO/tests/m3d/run_verification.sh" || slice_status=$?
...
echo
echo "=== 14. The five golden journeys ==="
journey_status=0
