#!/usr/bin/env bash
# M5a — the outlet keeps working when the cloud does not. Chains from OP-D, which chains
# the rest.
#
# The order matters for the usual reason and for one more this gate adds: M5a's suite
# proves the durable print queue on a REAL receipt, which means an order, a bill, a
# payment and a settlement have to have happened first. It borrows tests/m4c's helpers to
# build one rather than writing its own, because a second way to settle a bill is a second
...
bash "$REPO/tests/opd/run_verification.sh"
...
echo
echo "=== 22. M5a verification gates ==="
"$PY_BIN" "$REPO/tests/m5a/verify_m5a.py"
