#!/usr/bin/env bash
# M6-D — reporting, exports, and two closures that came due. Chains from M6-C.
#
# After the destructive slice rather than before it, because M6-C drops and recreates a
# database and this one reads the live floor's tickets. Nothing here destroys anything;
# every probe rolls back.
set -euo pipefail
...
bash "$REPO/tests/m6c/run_verification.sh"
...
echo
echo
echo "=== 27. M6-D verification gates ==="
"$PY_BIN" "$REPO/tests/m6d/verify_m6d.py"
